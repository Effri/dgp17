...
<?//p($arResult);?>
<form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" class="l-footer-form" id="footer">
	<?=bitrix_sessid_post()?>
    ...
    foreach ($arResult["PROPERTY_LIST"] as $propertyID):?>
        ...
        <?else:?>
            ...
            <?elseif($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "S" && !isset($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"])):?>
                <label><input type="email" placeholder="<?=$placeholder?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=$value?>"></label>
            <?endif?>
        <?endif;?>
    <?endforeach;?>
    ...
    <?endif?>
    <input type="submit" name="iblock_submit" class="btn btn-blue l-footer-submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" />
</form>
