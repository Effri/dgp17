...
<div class="mibok-wrapper-form" <?if(!empty($arResult['ERRORS'])):?>style="display:block;"<?endif;?>>
    <form name="iblock_add" action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" >
        <h4 class='mibok-form-name'><?=GetMessage('TIMETABLE_FAST_RECORD')?></h4>
        <?=bitrix_sessid_post()?>
            ...
            <?else:?>
                <div class="form-group" <?if($propertyID == 'PREVIEW_TEXT'):?>style="display:none;"<?endif;?>>
                    ...
                    <?elseif($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] == "S" && !isset($arResult["PROPERTY_LIST_FULL"][$propertyID]["USER_TYPE"])):?>
                        <input class="form-control" type="text"  name="PROPERTY[<?=$propertyID?>][<?=$i?>]" value="<?=$value?>">
                    <?endif?>
                </div>
            <?endif;?>
        <?endforeach;?>
        ...
        <div class="form-group">
            <input type="submit" name="iblock_submit" class="btn btn-default mibok-form-submit" value="<?=GetMessage("IBLOCK_FORM_SUBMIT")?>" disabled>
        </div>
