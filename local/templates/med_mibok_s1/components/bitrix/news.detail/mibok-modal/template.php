...
$day = MibokMed::ruDateFull("l", $arParams['TIME_INFO'][2]);

$nameLes = '';
if(strpos($arParams['MODAL_NAME'], 'PROPERTY_') === false)
    $nameLes = $arResult['FIELDS'][$arParams['MODAL_NAME']];
else
    $nameLes = $arResult['DISPLAY_PROPERTIES'][str_replace('PROPERTY_','',$arParams['MODAL_NAME'])]['DISPLAY_VALUE'];
...
<div class="modal-header">
    ...
    <h4 class="modal-title" id="myModalLabel"><?=$nameLes?></h4>
    <div class="modal-title-info">
        <span class="ic-m-t-clock modal-title-time"><?=$arParams['TIME_INFO'][0]?> - <?=$arParams['TIME_INFO'][1]?></span> <span class="modal-title-date"> <?=$date?> <?=$month?></span> <span class="modal-title-day"><?=$day?></span>
    </div>
</div>
        ...
        <table class="table">
            <?foreach($arResult["FIELDS"] as $code=>$value):
                ...
                <tr>
                    ...
                    {
                        ?>
                        <td><span class='modal-name-category'><?=GetMessage("IBLOCK_FIELD_".$code)?></span></td>
                            <?
            ...
            <?foreach($arResult["DISPLAY_PROPERTIES"] as $pid=>$arProperty):
                ...
                <tr>
                    <?if($pid == 'TEACHER' && !empty($arProperty['PHOTO']['src'])):?>
                        <td colspan="2">
                            ...
                            <div class="modal-teacher-info modal-teacher-text">
                                <span class='modal-name-category'><?=$arProperty["NAME"]?></span>
                    ...
                    <?elseif($pid == 'COLOR'):
                        continue;?>
                    <?else:?>
                        <td><span class='modal-name-category'><?=$arProperty["NAME"]?></span></td>
...
<?if(isset($arParams['IS_RECORD']) && $arParams['IS_RECORD'] =='Y'):?>
    <div class="modal-footer">
        <button type="button" class="btn btn-mibok mibok-show-form"><?=GetMessage('RECORD_LESSON')?></button>
...
<?else:?>
    <div class="modal-footer no-record"></div>
<?endif;?>
